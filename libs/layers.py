...
#                                     dropout=dropout,
#                                     activation_type=activation_type,)
        self.conv2= Conv3dResBlock(conv_dim1, conv_dim1,
                                    kernel_size=kernel_size,
                                    residual=residual,
                                    dropout=dropout,
                                    activation_type=activation_type,)
...
class LrkAttention(nn.Module):
    def __init__(self, n_head, d_model,pos_dim: int = 1, attention_type='Lrk'):
        super(LrkAttention, self).__init__()
        assert d_model % n_head == 0
        self.attention_type = attention_type
        self.d_k = d_model // n_head
        self.n_head = n_head
    ...
    def forward(self, query, key, value, pos=None, mask=None, weight=None, s=None, v = None, d = None):
        if mask is not None:
            mask = mask.unsqueeze(1)
        ...
        if pos is not None and self.pos_dim > 0:
            ...
            pos = pos.unsqueeze(1)
            pos = pos.repeat([1, self.n_head, 1, 1])
            value = torch.cat([pos, value], dim=-1)
#         d = (value@d).transpose(-1,-2)
#         s = value@s
        ...
        if pos is not None and self.pos_dim > 0:
            att_output = self.fc(att_output) 
        return att_output, torch.tensor([0])
...
class SpectralConv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        x_ft = torch.fft.rfftn(x, s=(n,n,n), norm = self.norm)
        x_ft = torch.stack([x_ft.real, x_ft.imag], dim=-1)
     
        out_ft = torch.zeros(batch_size, out_dim, n, n, n//2 + 1, 2, device=x.device)
                                               
        out_ft[:, :, :modes, :modes, :modes] = \
            self.compl_mul3d(x_ft[:, :, :modes, :modes, :modes], self.fourier_weight[0])
                                               
        out_ft[:, :, -modes:, :modes, :modes] = \
            self.compl_mul3d(x_ft[:, :, -modes:, :modes, :modes], self.fourier_weight[1])
                                               
        out_ft[:, :, :modes, -modes:, :modes] = \
            self.compl_mul3d(x_ft[:, :, :modes, -modes:, :modes], self.fourier_weight[2])
                                               
        out_ft[:, :, -modes:, -modes:, :modes] = \
            self.compl_mul3d(x_ft[:, :, -modes:, -modes:, :modes], self.fourier_weight[3])
                                               
        out_ft = torch.complex(out_ft[..., 0], out_ft[..., 1])
        #Return to physical space
        x = torch.fft.irfftn(out_ft, s=(n,n,n),norm = self.norm)
